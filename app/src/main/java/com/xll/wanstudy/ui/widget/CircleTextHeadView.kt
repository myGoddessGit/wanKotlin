...
class CircleTextHeadView(context: Context, attributeSet: AttributeSet) :
    ...
    init {
        val ta : TypedArray = context.obtainStyledAttributes(attributeSet, R.styleable.CircleTextHeadView, 0, 0)
    ...
    override fun onDraw(canvas: Canvas?) {
        canvas?.drawCircle((width / 2).toFloat(), (height / 2).toFloat(), (width / 2).toFloat(), borderPaint)
        super.onDraw(canvas)
    }
}